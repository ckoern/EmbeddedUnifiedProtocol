...
class FakeTransport:
    ...
    def queue_reply(self, payload):
        self.rx += eup.encode_frame(eup.MessageType.REPLY, payload)

    def queue_stream(self, payload):
        self.rx += eup.encode_frame(eup.MessageType.STREAM, payload)
    ...
    def last_command(self):
        ...
        self.tx.clear()
        return eup.decode_region(regions[-1])
...
def main():
    ...
    t.queue_reply(bytes([eup.StatusCode.Ok.value]) + struct.pack("<H", 42))
    status, value = dev.read_adc(2)
    check(status == eup.StatusCode.Ok, f"read_adc status {status}")
    check(value == 42, f"read_adc value {value}")
    sent = t.last_command()
    check(sent.type == eup.MessageType.COMMAND, "read_adc frame type")
    check(sent.payload == bytes([0x11, 2]), f"read_adc payload {sent.payload!r}")

    # --- status-only command: set_rgb(u8,u8,u8) -> (status,) ---
    t.queue_reply(bytes([eup.StatusCode.Ok.value]))
    (status,) = dev.set_rgb(255, 128, 0)
    check(status == eup.StatusCode.Ok, "set_rgb status")
    check(t.last_command().payload == bytes([0x10, 255, 128, 0]), "set_rgb payload")

    # --- InlineString result maps to bytes: get_name() -> (status, bytes) ---
    name = b"sensor-A"
    t.queue_reply(bytes([eup.StatusCode.Ok.value, len(name)]) + name)
    status, got = dev.get_name()
    check(status == eup.StatusCode.Ok, "get_name status")
    ...
    check(status == eup.StatusCode.Ok, "set_name(bytes) status")
    ...
    t.queue_reply(bytes([eup.StatusCode.Ok.value]) + span)
    status, block = dev.read_block(0)
    check(status == eup.StatusCode.Ok, "read_block status")
    ...
    wire = eup.encode_frame(eup.MessageType.COMMAND, b"\x11\x05")
    f = eup.decode_region(wire[:-1])
    check(f.type == eup.MessageType.COMMAND and f.payload == b"\x11\x05", "frame round trip")
